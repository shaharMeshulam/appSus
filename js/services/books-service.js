...
function getBooks(filterBy) {
    if (filterBy) {
        let { name, startPrice, endPrice } = filterBy
        startPrice = startPrice ? startPrice : 0
        endPrice = endPrice ? endPrice : Infinity
        const booksToShow = gBooks.filter(book => book.title.includes(name) && book.listPrice.amount >= startPrice && book.listPrice.amount <= endPrice)
...
function deleteReview(bookId, reviewId) {
    getBookById(bookId).then(book => {
        const currReviewIdx = book.reviews.findIndex(review => review.id === reviewId)
